...
def nested_to_device(data: Dict, device: torch.device) -> Dict:
...
class Launcher:
    def __init__(self, config: LauncherConfig) -> None:
        ...
        self.ckpt_dir = f"{self.output_dir}/ckpts"
        os.makedirs(self.ckpt_dir, exist_ok=True)
        self.stats_dir = f"{self.output_dir}/stats"
        os.makedirs(self.stats_dir, exist_ok=True)
        self.visual_dir = f"{self.output_dir}/visuals"
        os.makedirs(self.visual_dir, exist_ok=True)
        self.test_dir = f"{self.output_dir}/tests"
    ...
    def maybe_resume(self, state: Any) -> int:
        ...
        if self.config.resume:
            ...
            ckpt = torch.load(self.config.resume, map_location="cpu", weights_only=True)
            ...
            self.print_on_master(
                f"Resuming from ckpt: {self.config.resume}. set step to: {step}"
            )
        return step
